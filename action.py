...
class action() :
    ...
    def segment_distance(self, a, b):
        ...
        # 交差していれば距離0
        if self.segments_intersect(as_, ae, bs, be):
            return 0.0

        return min(
            self.point_segment_distance(as_, bs, be),
            self.point_segment_distance(ae, bs, be),
            self.point_segment_distance(bs, as_, ae),
            self.point_segment_distance(be, as_, ae),
        )
    ...
    def judge_closs_arms(self, now_frame) :
        ...
        left_center = [left_elbow[0] + left_wrist[0] / 2, left_elbow[1] + left_wrist[1] / 2]
        right_center = [right_elbow[0] + right_wrist[0] / 2, right_elbow[1] + right_wrist[1] / 2]

        center_distance = math.sqrt((left_center[0] - right_center[0]) ** 2 + (left_center[1] - right_center[1]) ** 2)
        ...
        if (left_horiz and right_vert) or (right_horiz and left_vert):
            if self.segment_distance([left_elbow, left_wrist], [right_elbow, right_wrist]) <= 0.1 :
                return True
        else :
            False